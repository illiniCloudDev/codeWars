...
function counterEffect(s) {
 //set the string to an array
  //set the substrings into numbers using Number()
  //set 4 variables for each one? 
  //for loop for each array 
  
  let sToArray = s.split('');
  let officialNumberArray = [];
  let indexZero = [];
  let indexOne = [];
  let indexTwo = [];
  let indexThree = [];
  ...
  //4 different for loops? 
  //.map()? 
  sToArray.forEach(element => {
    officialNumberArray.push(Number(element))
  })
  
  for(let i = 0; i <= officialNumberArray[0]; i++){
    indexZero.push(i)    
  }
    for(let i = 0; i <= officialNumberArray[1]; i++){
    indexOne.push(i)    
  }
    for(let i = 0; i <= officialNumberArray[2]; i++){
    indexTwo.push(i)    
  }
    for(let i = 0; i <= officialNumberArray[3]; i++){
    indexThree.push(i)    
  }
  
  finalAnswer.push(indexZero)
  finalAnswer.push(indexOne)
  finalAnswer.push(indexTwo)
  finalAnswer.push(indexThree)
